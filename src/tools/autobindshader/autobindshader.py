...
fcpp.write('		std::string msCode() {')
fcpp.write('		  std::string ms_code;')
if os.path.isfile(path+".ms"):
	fcpp.write('		  ms_code = ')
	importFile( path+".ms" );
	parseVars( path+".ms", False )
fcpp.write('	return ms_code; }\n')
...
if os.path.isfile(path+".vp"):
	if isComputeShader:
		...
		sys.exit(-1)
	if isMeshShader:
		print(' [AutoBindShader] FATAL ERROR, cannot mix mesh shader (ms) and rendering pipeline shaders (vp)')
		sys.exit(-1)
	fcpp.write('		  vp_code = ')
	importFile( path+".vp" )
...
fcpp.write('		std::string tsCode() {')
fcpp.write('		  std::string ts_code;')
if os.path.isfile(path+".ts"):
	if not isMeshShader:
		print(' [AutoBindShader] FATAL ERROR, a task shader (ts) always needs a maesh shader (ms)')
		sys.exit(-1)
	fcpp.write('		  ts_code = \n')
	importFile( path+".ts" )
	parseVars( path+".ts", False )
fcpp.write('	return ts_code; }\n')
...
if os.path.isfile(path+".gs"):
	if isComputeShader:
		...
		sys.exit(-1)
	if isMeshShader:
		print(' [AutoBindShader] FATAL ERROR, cannot mix mesh shader (ms) and rendering pipeline shaders (gs)')
		sys.exit(-1)
